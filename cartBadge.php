        ...
        if ($_SESSION['badgeCount'] > 0) {
          foreach ($_SESSION['badgeOrders'] as $row) {
            ...
            if(isset($row) && !empty($row)){
              ...
              $badgeOrderFilename = '';
              if($row -> getFilename() != ''){
                $badgeOrderFilename = $row -> getFilename();
              }
              else {
                $badgeOrderFilename = 'No File Uploaded';
              }
              ...
              $string .=
                  ...
                  "<input name='itemBadge[{$row -> getIndex()}][badgeImage]' id='itemBadge[{$row -> getIndex()}][badgeImage]' class='form-control' type='file' accept='image/jpeg, image/png' required onchange='onChangeFileUpload({$row -> getIndex()})' />" .
                  "<p class='text-start fs--1'>Current File: <span class='text-info' id='itemBadge[{$row -> getIndex()}][badgeImageName]'>{$badgeOrderFilename}</span></p>" .
                  "</div>" .

                  // badge type
                  "<div class='form-floating col-md-3 col-sm-12'>" .
                  "<label for='itemBadge[{$row -> getIndex()}][badgeType]' class='ms-2'>Badge Type</label>" .
                  "<select name='itemBadge[{$row -> getIndex()}][badgeType]' id='itemBadge[{$row -> getIndex()}][badgeType]' class='form-select' onchange='onChangeAjax(1, {$row -> getIndex()}, \"type\", this.value)' required >" .
              ...
              $string .=
                  ...
                  "<select name='itemBadge[{$row -> getIndex()}][badgeSize]' id='itemBadge[{$row -> getIndex()}][badgeSize]' class='form-select' onchange='onChangeAjax(1, {$row -> getIndex()},\"size\", this.value)' required >" .
              ...
              $string .=
                  ...
                  "<input name='itemBadge[{$row -> getIndex()}][badgeQty]' id='itemBadge[{$row -> getIndex()}][badgeQty]' type='number' class='form-control' value='{$row -> getQty()}' min='1' onchange='onChangeAjax(1, {$row -> getIndex()}, \"qty\", this.value)' required />" .
                  "</div>";
